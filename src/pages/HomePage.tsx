...
export default function HomePage() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        console.error("Nem sikerült betölteni a sztorikat:", error);
      }
    })();
  }, [client]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen w-screen" style={{ background: 'var(--page-bg)' }}>
      
      <p className="title antialiased italic fill-cyan-500 drop-shadow-lg drop-shadow-cyan-500/50 mb-6 text-4xl">
        <b><strong>evo<a className="decoration-sky-600 noHover">Story</a></strong></b>
      </p>
      
      <div className="border-3 border-gray-400 p-8 mx-auto w-4/5 max-w-lg rounded-lg drop-shadow-lg drop-shadow-indigo-500/50 flex flex-col gap-6" style={{ backgroundColor: 'var(--box-bg-light)' }}>
        
        {localStorage.getItem("lastSceneId") && (
        ...
        <div className="flex flex-col gap-3">
            <p className="text-center font-bold text-lg mb-2">{t('homePage.title') || "Choose an Adventure"}</p>
            
            {stories.length > 0 ? (
                stories.map((story) => (
                    <div key={story.id} className="flex justify-between items-center bg-gray-800/50 p-3 rounded-lg border border-gray-600 hover:border-cyan-500 transition">
                        <span className="font-semibold text-white ml-2">{story.title}</span>
                        <Button
                ...
                <p className="text-center text-gray-400">Loading stories...</p>
            )}
        </div>
